...
public abstract class BluetoothActivity extends AppCompatActivity {
    ...
    public boolean write(String s) {
        if (isConnected) {
            try {
                btService.send(Message.obtain(null, BluetoothService.MESSAGE_WRITE, s));
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
    ...
    public void onRead(String s) {
        ...
        while ((index = stringBuffer.indexOf(bufferUntilStr)) != -1) {
            for (OnBluetoothEventListener listener : listeners) {
                listener.onReadLine(stringBuffer.substring(0, index));
            }
            stringBuffer.delete(0, index + 1);
        }
    }

    public void setBufferUntil(String s) {
        bufferUntilStr = s;
    }
